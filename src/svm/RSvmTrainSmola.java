...
public class RSvmTrainSmola {
	...
	public RSvmTrainSmola(Data data, double C_, double tau_, double eps_,
	...
	public void train() {
		...
		smo = new RSmo1Smola(kernel, label, C, tau, eps);
	...
	double classify(InnerProductSpace x) {
		...
		for (i = 0; i < l; i++) {
			a = alpha[i] - alpha_s[i];
			if (Math.abs(a) > tau) {
				val += a * kernel.value(this.point[i], x);
			}
		}
	...
	double standarDeviation(Data data) {
		double s, m, val;
		int i;
	...
	public static void main(String[] argv) throws Exception {
		...
		svm = new RSvmTrainSmola(data, C, tau, eps, type, degree, factor, bias,
				size);
		svm.header = header;
