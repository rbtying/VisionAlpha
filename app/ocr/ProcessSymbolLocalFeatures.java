...
public class ProcessSymbolLocalFeatures {
	static int np = 10, nr = 32, nn = 3;
				...
				try {
					if (actualSymbol.size() == 1)
						fileout1.writeBytes(symbolNames.get(j) + " "
								+ actualSymbol.getProcessed(nr, np, nn) + "\n");
	...
	public static void main(String[] argv) throws Exception {
		nr = Integer.parseInt(argv[0]);
		np = Integer.parseInt(argv[1]);
		nn = Integer.parseInt(argv[2]);
		if (argv.length == 5)
			convert(argv[3], argv[4]);
		else if (argv.length == 6)
			convert(argv[3], argv[4], argv[5]);
		else
